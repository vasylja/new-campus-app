...
public class LectureItemManager extends SQLiteOpenHelper {
	...
	public void importFromTUMOnline(Context con) throws Exception {
		...
		try {
			...
			FindLecturesRowSet MyLecturesList = new FindLecturesRowSet();
			MyLecturesList.setLehrveranstaltungen(new ArrayList<FindLecturesRow>());

			try {
				MyLecturesList = serializer.read(FindLecturesRowSet.class, strMine);
			} catch (Exception e) {
